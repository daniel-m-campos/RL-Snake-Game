...
template <typename S, typename A> class MockLearner : public Learner<S, A>
{
  public:
    MOCK_METHOD(void, reinforce,
                ((Policy<S, A> const &), (ActionValuer<S, A> &), S, A, double, S),
                (override));
};
...
TEST_F(AgentImplFixture, TestUpdate)
{
    ...
    EXPECT_CALL(*learner_ptr.get(), reinforce(_, _, state, action, reward, new_state))
        .WillOnce(Return());
...
TEST_F(AgentImplFixture, TestGetAction)
{
    ...
    AgentImpl<int, int> agent{valuer_ptr, std::move(policy_ptr), std::move(learner_ptr),
                              state, action};
    EXPECT_EQ(agent.get_action(new_state), new_action);
}
