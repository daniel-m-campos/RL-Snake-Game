    ...
    -> std::shared_ptr<GameAV>
{
    ...
    using key_type = std::pair<GameState, snake::Direction>;

    std::unordered_map<key_type, double> max_map;
    ...
    // Build the merged action-value map and collect all unique states
    std::set<GameState> states;
    SimpleGameAV::action_value_map merged_map;
    for (auto &[key, value] : max_map)
    {
...
static void run_chunk(std::vector<WorkerState> &workers, TrainingConfig const &config,
                      size_t num_threads, int64_t episodes_this_chunk,
                      std::shared_ptr<TrainingProgress> const &progress)
{
    ...
    for (size_t i = 0; i < num_threads; ++i)
    {
        ...
        threads.emplace_back([&workers, &config, &progress, i, episodes]()
                    ...
                    std::shared_ptr<TrainingProgress> progress)
{
    ...
    while (episodes_done < config.num_episodes)
    {
        ...
        run_chunk(workers, config, num_threads, episodes_this_chunk, progress);
